...
impl SelectedShape{
    ...
    fn coord_state(&self, coord_ref: &Rc<RefCell<Coord>>)->CoordState{
        match &self.hover_coord {
            ...
            _ => {
                match self.coords.iter().find(|coord| *coord == coord_ref){
...
pub fn change_hover(scene: &mut Scene, cursor_position: Point) {
    ...
    'shape_loop: for shape_selected in &mut scene.selected.shapes{
        let shape = scene.vgc_data.get_shape(shape_selected.shape_index).unwrap();
...
pub fn draw(scene: &Scene, frame: &mut Frame) {
    ...
    for shape_selected in &scene.selected.shapes{
        ...
        shape.visit_full_curves( |_, p0, cp0, cp1, p1| {
            let from = Point::new(
                p0.x,
                p0.y * 1.0 / scene.vgc_data.ratio as f32,
            );
            ...
            let from = Point::new(
                cp1.x,
                cp1.y * 1.0 / scene.vgc_data.ratio as f32,
            );
        ...
        let path = Path::new(|p| {
            ...
            shape.visit_full_curves(move |_, _, cp0, cp1, p1| {
                p.bezier_curve_to(
                    Point::new(
                        cp0.x,
                        cp0.y * 1.0 / scene.vgc_data.ratio as f32,
                    ),
                    Point::new(
                        cp1.x,
                        cp1.y * 1.0 / scene.vgc_data.ratio as f32,
                    ...
                    Point::new(
                        p1.x,
                        p1.y * 1.0 / scene.vgc_data.ratio as f32,
                    )
                );
            });
        });
...
pub fn draw_closest_pt(scene: &Scene, frame: &mut Frame, pos: Point)  {
    ...
    let center = Point::new(
        min_coord.x,
        min_coord.y * 1.0 / scene.vgc_data.ratio as f32,
    );
    frame.fill(
