...
impl ShapeSolid {
    ...
    pub fn get_curve(&self, index: usize) -> Option<CurveSolid> {
    ...
    fn is_handles_joined(&self, index: usize) -> bool {
        let curve = self.get_curve(index).expect("Curve should exist");
    ...
    pub fn insert_coord_smooth(&mut self, curve_index: usize, t: f32) {
        ...
        let (_, cp1l, p1, cp1r, _) = curve::add_smooth_result(
            &curve.p0.coord,
            &curve.cp0.coord,
            &curve.cp1.coord,
            &curve.p1.coord,
...
impl VgcDataBase {
    ...
    pub fn shape_diff_apply(&mut self, new_shape: ShapeSolid) {
        ...
        //under construction
        // Build shape diff like a text diff
        // apply the diff like insert x at index y etc.
    }
}
