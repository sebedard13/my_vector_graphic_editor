...
export class SelectionService {
    ...
    constructor(
        ...
        eventsService.mouseDown$.pipe(filter((event) => event.buttons == 1)).subscribe((event) => {
            this.scenesService.currentSceneNow((scene) => {
                if (event.shiftKey) {
                    const point = scene.canvasContent.get_project_mouse(new ScreenCoord(event.offsetX, event.offsetY));
        ...
        eventsService.mouseMove$.subscribe((event) => {
            this.scenesService.currentSceneNow((scene) => {
                const pt = scene.canvasContent.get_project_mouse(new ScreenCoord(event.offsetX, event.offsetY));
